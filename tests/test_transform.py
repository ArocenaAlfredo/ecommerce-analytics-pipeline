...
def test_query_delivery_date_difference(database: Engine):
    ...
    expected = {obj["State"]: int(obj["Delivery_Difference"]) for obj in expected}
    actual = {
        obj["State"]: int(obj["Delivery_Difference"])
        for obj in pandas_to_json_object(actual.result)
    }

    assert len(actual) == len(expected)
    for state in expected:
        assert math.isclose(expected[state], actual[state], abs_tol=1)
